...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def unload_models(self, model_keys: List[str]):
        ...
        for key in model_keys:
            if key in self.models:
                self.models[key].cpu()  # Use .cpu() instead of .to("cpu")
                if hasattr(self.models[key], 'parameters'):
    ...
    def run(
        ...
        # Sample sparse structure with progress tracking
        coords = self.sample_sparse_structure(cond, num_samples, sparse_structure_sampler_params)
        if progress_callback:
            for _ in range(sparse_structure_sampler_params.get('steps', 8)):
                progress_callback()
                
        # Sample slat with progress tracking
        slat = self.sample_slat(cond, coords, slat_sampler_params)
        if progress_callback:
            for _ in range(slat_sampler_params.get('steps', 16)):
                progress_callback()
